...
class Emulation:
    ...
    def configure_traffic(self, traffic_config=None):
        ...
        for flowconfig in self.traffic_config:
            ...
            # elif protocol == 'cubic':
            elif CC_in_kernel(protocol):
                # Create server start up call
                params = (destination,source_node,duration)
    ...
    def run(self):
        ...
        for call in self.call_second:
            print("sleep time=",call.waiting_time)
            time.sleep(call.waiting_time)
            call.command(*call.params)
    def run_countinued(self):
        for node_name in self.waitoutput:
            host = self.network.get(node_name)
            print("waitoutput ",node_name)
            output = host.waitOutput()
            
            mkdirp(self.path)
            if (node_name == "c1" or node_name == "x1"):
                if self.ff==1:
                    continue
    ...
    def start_ffmpeg_sender(self,inputfile,senderip,node_name):
        mkdirp(self.path)
        loglocation = '%s/%s_output.txt' % (self.path, node_name)
        ffcmd = f"ffmpeg -re -i {inputfile} -c copy -stats_period 1 -f mpegts tcp://{senderip}:5201?listen > {loglocation} 2>&1 &"
        # ffcmd = f"ffmpeg -re -i {inputfile} -c copy -stats_period 1 -f mpegts tcp://{senderip}:5201?listen &"
        return ffcmd

    def start_ffmpeg_reveiver(self,senderip,node_name,duration):
        mkdirp(self.path)
        loglocation = '%s/%s_output.txt' % (self.path, node_name)
        ffcmd = f"sleep 2; ffplay -t {duration} -autoexit -i tcp://{senderip}:5201 > {loglocation} 2>&1"
        return ffcmd

    def start_iperf_server(self, node_name, destination_name, duration, port=5201, monitor_interval=1):
        ...
        if self.ff==1 and (node_name == "x1" or node_name == "c1"):
            cmd  = self.start_ffmpeg_reveiver(destination.IP(),node_name,duration)
            
        print("Sending command '%s' to host %s %s" % (cmd, node.name,node.IP()))
        
        node.sendCmd(cmd)

    def start_iperf_client(self, node_name, destination_name, duration, CCA="cubic", port=5201, monitor_interval=1):
        node = self.network.get(node_name)
        destination =  self.network.get(destination_name)
        iperfArgs = 'iperf3 -p %d -i %s --json --linux-congestion %s ' % (port, monitor_interval,CCA)
        cmd = iperfArgs + '-t %d -c %s' % (duration, destination.IP())
        if self.ff==1 and (node_name == "x1" or node_name == "c1"):
            cmd = self.start_ffmpeg_sender(f'12minput{duration}.mp4',node.IP(),node_name)
        if self.ff==2 and (node_name == "x1" or node_name == "c1"):
            cmd = iperfArgs + '-t %d -c %s -b 12M' % (duration, destination.IP())
        print("Sending command '%s' to host %s %s" % (cmd, node.name,node.IP()))
    ...
    def adjust_tc_netem_loss_rate(self,new_rate:str,wait_time=0):
        if wait_time > 0:
            print("wait time=",wait_time)
            time.sleep(wait_time)
        if len(self.tc_loss_cmds) == 1:
            for tc_cmd in self.tc_loss_cmds:
                loss_position = tc_cmd.find("loss")
                new_cmd = tc_cmd[:loss_position]+" loss %s%%" % (new_rate)
                new_cmd = new_cmd.replace("add","change")
                print("Running the following command in root terminal:",new_cmd)
                os.system(new_cmd)
        else:
            print("why?")
